class EmployeesController < ApplicationController
  ...
  def results
    ...
    @employees_less_orders = Employee.where(age: ..24).or(Employee.order(:id).where(no_of_order: 5))
    @employees_desc = Employee.order(age: :desc)
    @employees_asc = Employee.order(:no_of_order)
    @employees_high = Employee.where(salary: 45001..)
    @employees_group = Employee.select(:id, :first_name, :no_of_order).group(:id, :first_name, :no_of_order).where(no_of_order: 6..)
